...
- [x] ターミナル内検索 (`Ctrl+Shift+F`, xterm.js search addon)
- [x] コピー&ペースト改善 (選択時自動コピー, 右クリックペースト)
- [x] ドラッグ&ドロップ — ファイルブラウザからターミナルへパス入力
...
- [x] 画像プレビュー (モーダル表示)
- [x] PDF プレビュー (blob URL + iframe)
- [x] Markdown プレビュー (marked + DOMPurify)
...
- [x] 全新機能の i18n キー追加 (検索, スクロールバック, SSH, サーバー, ファイルエディタ)
...
- [x] 通知のオン/オフ設定
- [x] マルチサーバー対応 (サーバープロファイル管理 + 切り替え)
- [x] スクロールバックパネル (tmux capture-pane + API)
- [x] SSH クイック接続 (プリセット管理 + 自動実行)
