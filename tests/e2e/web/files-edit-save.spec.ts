...
test.beforeAll(async () => {
  ...
  ({ home, env } = createGatewayEnv({ port: PORT, token: TOKEN, label: 'zenterm-files-editsave' }));
...
test('edit and save: replaces file content on disk', async ({ page }) => {
  ...
  const onDisk = readFileSync(join(home, 'edit.txt'), 'utf-8');
